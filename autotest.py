...
RUNS = int(os.getenv('RUNS', default=50))
HOST = os.getenv('HOST', default='192.168.0.142')
OUT_LOCK = Lock()
DISABLE_LOG = False
...
class Checker(BaseValidator):
    ...
    def _get_info(self):
        ...
        # RuCTF format: "vulns: 2:1:2"
        if out.startswith("vulns: "):
            info = {
                "vulns": out.count(":"),
                "timeout": 30,
                "attack_data": False,
    ...
    def _run_command(self, command: List[str], env=None) -> Tuple[str, str]:
        ...
        try:
            ...
            pass
        # хз, походу какой то чек отваливается по таймауту, возможно надо ограничить кол-во паказываемых постов
